...
import { NEWS_ARTICLES, DISTRICTS } from '../data/mockData';
import NewsCard from '../components/NewsCard';

const HomePage: React.FC = () => {
  const generalNews = NEWS_ARTICLES.filter(article => !article.districtId).slice(0, 3);
  ...
  return (
    <div className="space-y-16">
      ...
      <section 
        ...
        style={{ backgroundImage: `url('https://picsum.photos/seed/sl-gov/1200/400')` }}
      >
        <div className="absolute inset-0 bg-blue-900 opacity-60 rounded-lg"></div>
        <div className="relative z-10">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            District Services Portal of Sri Lanka
          </h1>
          <p className="text-lg max-w-3xl">
            Your official gateway to services, information, and news across all administrative districts.
          </p>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center">
          <div className="md:w-3/4">
            ...
            <p className="text-slate-700 leading-relaxed">
              Welcome to the unified District Services Portal. This platform is a significant step towards our vision of a digitally empowered nation. We are committed to providing transparent, efficient, and accessible services to every citizen. Through this portal, we aim to bring governance closer to the people, ensuring that you can access the information and services you need with ease and confidence. We encourage you to explore the resources available for your district and engage with us as we continue to enhance this platform.
            </p>
          </div>
      ...
      {/* Districts Section */}
      <section>
        <h2 className="text-3xl font-bold mb-8 text-center text-slate-800">Explore Our Districts</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 text-center">
            {DISTRICTS.map(d => (
                <Link to={`/districts/${d.id}`} key={d.id} className="p-4 bg-white border border-slate-200 rounded-lg hover:shadow-md hover:border-blue-500 transition-all">
                    <span className="font-semibold text-slate-700">{d.name}</span>
                </Link>
            ))}
             <Link to="/districts" className="p-4 bg-slate-100 border border-slate-200 rounded-lg hover:shadow-md hover:border-blue-500 transition-all flex items-center justify-center">
